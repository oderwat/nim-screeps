...
proc handleTower*(tower: StructureTower) =
  ...
  # hostiles? who do we want to hit?
  var closestHostile: Creep
  # we search for healers only every other tick
  if game.time mod 2 == 1:
    # looking for healers in the hostiles and make them first target
    closestHostile = tower.findClosestHostileByRange(Creep) do(creep: Creep) -> bool:
      for p in creep.body:
        if p.part == Heal:
          return true
      return false
  ...
  # this is the more peacefull code
